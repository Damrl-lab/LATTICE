...
#include "cont.h"
// #define CP_BACKGROUND

//#define PROFILE

int check_point_phase = 0;
...
void do_cp_file(void *ptr)
{
    checkpoint_args *args = (checkpoint_args *)ptr;
#ifdef GPU
    if (args->net->gpu_index >= 0)
    {
    ...
    (*args->net->seen) = get_current_net_iteration(args->net) * args->net->batch * args->net->subdivisions; // remove this line, when you will save to weights-file both: seen & cur_iteration
    fwrite(args->net->seen, sizeof(uint64_t), 1, fp);

    int i;
    for (i = 0; i < args->net->n && i < args->cutoff; ++i)
    {
...
void save_weights_upto(network net, char *filename, int cutoff, int save_ema, double *pull_time, double *cp_time)
{
    ...
    *pull_time = (end_time - st_time);
    if (check_point_phase != NO_CP_ONGOING)
    {
        printf("File CP wait\n");
        while(check_point_phase != NO_CP_ONGOING);
    }
    check_point_phase = CP_COPY;
    struct checkpoint_args *cp_args;
    cp_args = (checkpoint_args *)malloc(sizeof(checkpoint_args));
    cp_args->pull_time = pull_time;
    cp_args->cutoff = &cutoff;
    cp_args->filename = filename;
    cp_args->save_ema = &save_ema;
    cp_args->net = &net;
    cp_args->cp_time = cp_time;
    cp_args->cp_phase = &check_point_phase;


st_time = what_time_is_it_now();
#ifdef CP_BACKGROUND
    pthread_t thread;
    pthread_t load_thread = pthread_create(&thread, 0, do_cp_file, cp_args);
#else
    do_cp_file(cp_args);
#endif
*cp_time =  what_time_is_it_now() - st_time;
...
void *do_cp(void *ptr)
{
...
#ifdef PROFILE
    //nvtxNameOsThread(pthread_self(),"CHEKPOINT");
    //nvtxRangePush(pthread_self(__FUNCTION__);
    // nvtxMark("Waiting...");
#endif
    checkpoint_args *args = (checkpoint_args *)ptr;
    ...
    double end_time = 0;
    st_time = what_time_is_it_now();
    network_libpm *network_libpm_ptr;
    int i, j;
    if (args->is_initial_checkpoint == 1)
    {
        network_libpm_ptr = (network_libpm *)container_palloc(args->cont->id, sizeof(*network_libpm_ptr));
        network_libpm_ptr->major = MAJOR_VERSION;
        network_libpm_ptr->minor = MINOR_VERSION;
        network_libpm_ptr->revision = PATCH_VERSION;
        network_libpm_ptr->seen = *args->net->cur_iteration * args->net->batch * args->net->subdivisions;
        int layer_count = 0;
        for (i = 0; i < args->net->n && i < args->cutoff; ++i)
        {
            layer *l = &args->net->layers[i];
        ...
        network_libpm_ptr->layers_libpm = (layer_libpm **)xcalloc(layer_count, sizeof(layer_libpm *));
        mallocat(network_libpm_ptr->layers_libpm, layer_count * sizeof(struct layer_libpm));
        pointerat(args->cont->id, &network_libpm_ptr->layers_libpm);
        for (i = 0, j = 0; j < args->net->n && j < args->cutoff; ++j)
        {
            layer *l = &args->net->layers[j];
            layer l_obj = args->net->layers[j];
            if (l->type == CONVOLUTIONAL && l->share_layer == NULL)
            {
#ifdef GPU
                if (gpu_index >= 0)
                {
                    double temp_st = what_time_is_it_now();
                    pull_convolutional_layer(l_obj);
                    double temp_end = what_time_is_it_now();
                    *args->pull_time += (temp_end - temp_st);
                }
#endif

                network_libpm_ptr->layers_libpm[i] = (layer_libpm *)malloc(sizeof(layer_libpm));
                mallocat(network_libpm_ptr->layers_libpm[i], sizeof(struct layer_libpm));
                pointerat(args->cont->id, &network_libpm_ptr->layers_libpm[i]);
                int num = l->nweights;

                network_libpm_ptr->layers_libpm[i]->biases = (float *)l->biases;
                mallocat(network_libpm_ptr->layers_libpm[i]->biases, sizeof(float) * l->n);
                pointerat(args->cont->id, &network_libpm_ptr->layers_libpm[i]->biases);

                if (l->batch_normalize)
                {
                    network_libpm_ptr->layers_libpm[i]->scales = (float *)l->scales;
                    mallocat(network_libpm_ptr->layers_libpm[i]->scales, sizeof(float) * l->n);
                    pointerat(args->cont->id, &network_libpm_ptr->layers_libpm[i]->scales);

                    network_libpm_ptr->layers_libpm[i]->rolling_mean = (float *)l->rolling_mean;
                    mallocat(network_libpm_ptr->layers_libpm[i]->rolling_mean, sizeof(float) * l->n);
                    pointerat(args->cont->id, &network_libpm_ptr->layers_libpm[i]->rolling_mean);

                    network_libpm_ptr->layers_libpm[i]->rolling_variance = (float *)l->rolling_variance;
                    mallocat(network_libpm_ptr->layers_libpm[i]->rolling_variance, sizeof(float) * l->n);
                    pointerat(args->cont->id, &network_libpm_ptr->layers_libpm[i]->rolling_variance);
                }
                network_libpm_ptr->layers_libpm[i]->weights = (float *)l->weights;
                mallocat(network_libpm_ptr->layers_libpm[i]->weights, sizeof(float) * num);
                pointerat(args->cont->id, &network_libpm_ptr->layers_libpm[i]->weights);
                i++;
            }
            ...
            if (l->type == CONNECTED) {
                ...
                network_libpm_ptr->layers_libpm[i]->biases = (float *)l->biases;
                mallocat(network_libpm_ptr->layers_libpm[i]->biases, sizeof(float) * l->outputs);
                pointerat(args->cont->id, &network_libpm_ptr->layers_libpm[i]->biases);
                
                if (l->batch_normalize)
                {
                    network_libpm_ptr->layers_libpm[i]->scales = (float *)l->scales;
                    mallocat(network_libpm_ptr->layers_libpm[i]->scales, sizeof(float) * l->outputs);
                    pointerat(args->cont->id, &network_libpm_ptr->layers_libpm[i]->scales);
                
                    network_libpm_ptr->layers_libpm[i]->rolling_mean = (float *)l->rolling_mean;
                    mallocat(network_libpm_ptr->layers_libpm[i]->rolling_mean, sizeof(float) * l->outputs);
                    pointerat(args->cont->id, &network_libpm_ptr->layers_libpm[i]->rolling_mean);
                
                    network_libpm_ptr->layers_libpm[i]->rolling_variance = (float *)l->rolling_variance;
                    mallocat(network_libpm_ptr->layers_libpm[i]->rolling_variance, sizeof(float) * l->outputs);
                    pointerat(args->cont->id, &network_libpm_ptr->layers_libpm[i]->rolling_variance);
                }
                
                network_libpm_ptr->layers_libpm[i]->weights = (float *)l->weights;
                mallocat(network_libpm_ptr->layers_libpm[i]->weights, sizeof(float) * l->outputs * l->inputs);
                pointerat(args->cont->id, &network_libpm_ptr->layers_libpm[i]->weights);
        ...
        container_setroot(args->cont->id, network_libpm_ptr);
        pointerat(args->cont->id, &network_libpm_ptr->layers_libpm);
    }
    else
    {
        ...
        if (gpu_index >= 0)
        {
            ...
            {
                layer *l = &args->net->layers[i];
                if (args->skip_layers >= i) {
                    // printf("Layer %d skipped.\n", i);
                    continue;
                }
                if (l->type == CONVOLUTIONAL && l->share_layer == NULL)
                {
    ...
    double container_st = what_time_is_it_now();
    double flush_ts;
    // int pages_added = container_checkpoint_with_phase(args->cont->id, 1, args->cp_phase);
    int pages_added = container_checkpoint_with_phase(args->cont->id, 0, args->cp_phase);
    printf("Pages_Added: %d ", pages_added);
    double container_et = what_time_is_it_now();
    *args->flush_time = 0;
    *args->cp_time = container_et - container_st;

    if (args->is_initial_checkpoint)
    {

        for (i = 0, j = 0; i < args->net->n && i < args->cutoff; ++i)
        {
            layer *l = &args->net->layers[i];
            if (l->type == CONVOLUTIONAL && l->share_layer == NULL)
            {
                layer_libpm *ll = network_libpm_ptr->layers_libpm[j];
                int num = l->nweights;
                l->biases = network_libpm_ptr->layers_libpm[j]->biases;

                if (l->batch_normalize)
                {
                    l->scales = network_libpm_ptr->layers_libpm[j]->scales;
                    l->rolling_mean = network_libpm_ptr->layers_libpm[j]->rolling_mean;
                    l->rolling_variance = network_libpm_ptr->layers_libpm[j]->rolling_variance;
                }
                l->weights = network_libpm_ptr->layers_libpm[j]->weights;
                j++;
            }
...
int save_weights_upto_libpm(network net, char *filename, int cutoff, int save_ema, bool is_inital_checkpoint,
                            struct container *cont, unsigned int container_number, double *pull_time, double *cp_time, double *flush_time, int skip_layers)
{
    double st_time = what_time_is_it_now();
    double end_time = 0;
#ifdef GPU
    ...
    if (check_point_phase != NO_CP_ONGOING)
    {
        printf("Shadow Copy wait\n");
        while(check_point_phase != NO_CP_ONGOING);

    }
    check_point_phase = CP_COPY;
    int is_initial = is_inital_checkpoint;

    struct checkpoint_args *cp_args;
    cp_args = (checkpoint_args *)malloc(sizeof(checkpoint_args));
    cp_args->cont = cont;
    cp_args->container_number = &container_number;
    cp_args->pull_time = pull_time;
    cp_args->cutoff = &cutoff;
    cp_args->filename = filename;
    if (is_inital_checkpoint)
        cp_args->is_initial_checkpoint = 1;
    else
        cp_args->is_initial_checkpoint = 0;
    cp_args->save_ema = &save_ema;
    cp_args->cp_phase = &check_point_phase;
    cp_args->net = &net;
    cp_args->cp_time = cp_time;
    cp_args->flush_time = flush_time;
    cp_args->skip_layers = skip_layers;

#ifdef CP_BACKGROUND
    pthread_t thread;
    pthread_t load_thread = pthread_create(&thread, 0, do_cp, cp_args);
#else
    do_cp(cp_args);
#endif
    return 0;
}
int save_weights_libpm(network net, char *filename, bool is_initial_checkpoint, struct container *cont,
                       unsigned int container_num, double *pull_time, double *cp_time, double *flush_time, int skip_layers)
{
    return save_weights_upto_libpm(net, filename, net.n, 0, is_initial_checkpoint, cont, container_num, pull_time, cp_time, flush_time, skip_layers);
}
void save_weights(network net, char *filename)
{
    save_weights_2(net, filename, NULL, NULL);
}

void save_weights_2(network net, char *filename, double *pull_time, double *cp_time)
{
    save_weights_upto(net, filename, net.n, 0, pull_time, cp_time);
}
...
void load_weights_upto(network *net, char *filename, int cutoff)
{
   ...
   int transpose = (major > 1000) || (minor > 1000);
    ...
    end_time = what_time_is_it_now();
    cp_time = (end_time - st_time) - push_time;

    printf("File restore time: %f push_time;%f \n", cp_time, push_time);
}
...
void load_weights_upto_libpm(network *net, struct container *cont, int container_num, int cutoff)
{
    ...
    double push_time = 0;
    double cp_time = 0;
    cont = container_restore(container_num);
    network_libpm *network_libpm_ptr = container_getroot(container_num);

    if ((network_libpm_ptr->major * 10 + network_libpm_ptr->minor) >= 2)
    {
        *net->seen = network_libpm_ptr->seen;
    }
    else
    {
        *net->seen = network_libpm_ptr->seen;
    ...
    *net->cur_iteration = get_current_batch(*net);
    int transpose = (network_libpm_ptr->major > 1000) || (network_libpm_ptr->major > 1000);

    int i, j;
    for (i = 0, j = 0; i < net->n && i < cutoff; ++i)
    {
    ...
    end_time = what_time_is_it_now();
    cp_time = (end_time - st_time) - push_time;
    printf("LibPM restore time: %f push_time:%f \n", cp_time, push_time);
}

void load_weights_libpm(network *net, struct container *cont, unsigned int container_num)
{
    load_weights_upto_libpm(net, cont, container_num, net->n);
}
