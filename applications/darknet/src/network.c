...
int64_t get_current_net_iteration(network* net)
{
    return *net->cur_iteration;
}
...
float train_network_3(network net, data d, double* timestamp)
{
    return train_network_waitkey_3(net, d, 0,timestamp);
}

float train_network_2(network net, data d, double* fp_bp, double* wu)
{
    return train_network_waitkey_2(net, d, 0,fp_bp,wu);
}

float train_network_waitkey_2(network net, data d, int wait_key, double* fp_bp, double* wu)
{
...
float train_network_waitkey_3(network net, data d, int wait_key, double* timestamp)
{
    assert(d.X.rows % net.batch == 0);
    int batch = net.batch;
    int n = d.X.rows / batch;
