...
#define LIBLATTICE
#define CP_FREQ 1
#define FM_FILE_NAME_PREFIX "/mnt/pmem2/container"
#define FILE_CHECKPOINT_PATH "/mnt/nvme0n1"
...
{
    printf("checkpoint phase %d\n", CP_SHADOW_COPY);
    dontuse_opencv = 1; //hardcoded
    int i;
    float avg_loss = -1;
    ...
    unsigned int CONTAINER_NUMBER = 0;
    bool is_initial_checkpoint ;
    int checkpoint_count = 0;
    double total_cp_time = 0;
    double first_cp_time = 0;
    double total_pull_time = 0;
    double total_flush_time = 0;
    double total_push_time = 0;
    double total_data_prep_time = 0;
    double total_fw_bw = 0;
    double total_fp_bp = 0;
    double total_wu = 0;
    double total_fp_bp_cp = 0;
    double total_wu_cp = 0;
    double current_fp_bp_time = 0;
    double pull_time = 0;
    double cp_time = 0;
    double flush_time = 0;
    int cp_freq = CP_FREQ;
    ...
    for(i = 0; i < ngpus; ++i){
        ...
        if(weightfile){
                ...
                load_weights_libpm(&nets[i], cont,  CONTAINER_NUMBER);
                return;
            #else
            ...
            #ifdef LIBLATTICE
            delete_container();
            cont = container_initialization(CONTAINER_NUMBER);
            #endif
        }
    ...
    double total_trainning_time_start = what_time_is_it_now();
    while(get_current_batch(net) < net.max_batches || net.max_batches == 0){
        #ifdef PROFILE
        cudaProfilerStart();
        #endif
        double begin_time = what_time_is_it_now();
        double st_time = what_time_is_it_now();
        pthread_join(load_thread, 0);
        train = buffer;
        load_thread = load_data(args);
        double et_time = what_time_is_it_now();
        total_data_prep_time = total_data_prep_time + (et_time-st_time);
        float loss = 0;
        st_time = what_time_is_it_now();
        double timestamp_fw_bw;
        #ifdef GPU
                if(ngpus == 1){
                    //loss = train_network_2(net, train,&total_fp_bp, &total_wu);
                     loss = train_network_3(net, train,&timestamp_fw_bw);
                } else {
        ...
        #endif
        et_time = what_time_is_it_now();
        total_fp_bp = total_fp_bp + (timestamp_fw_bw - st_time);
        total_fp_bp_cp =  total_fp_bp_cp + (timestamp_fw_bw - st_time);
        total_wu = total_wu + (et_time  - timestamp_fw_bw);
        total_wu_cp +=  (et_time  - timestamp_fw_bw);
        ...
        if (i >= (iter_save_last + cp_freq)) {
            ...
            if(!is_initial_checkpoint)
             ...
             if(!is_initial_checkpoint)
            {
                total_cp_time += cp_time;
                total_pull_time += pull_time;
                total_flush_time += flush_time;
                printf("%d. cp time: %f, fp_bp time: %f, wu time: %f\n", 
                    checkpoint_count, cp_time, total_fp_bp_cp, total_wu_cp);
                total_fp_bp_cp  = 0;
                total_wu_cp = 0;
            ...
            if (checkpoint_count > 0 && skip_layers < net.n) {
                // skip_layers = -1;
                // skip_layers++;
                skip_layers = net.n - 5;
            }
            int new_pages = save_weights_libpm(net, buff,is_initial_checkpoint,cont,CONTAINER_NUMBER, &pull_time, &cp_time,&flush_time, skip_layers);
            #else            
            sprintf(buff, "%s/%s_final.weights", FILE_CHECKPOINT_PATH, base);
            save_weights_2(net, buff,&pull_time, &cp_time);
            #endif
            if(is_initial_checkpoint)
                first_cp_time  = cp_time;
            is_initial_checkpoint  = false;
        }
        free_data(train);
    ...
    printf("Total cp time: %f\n",total_cp_time);
    printf("AVG CP time: %f\n",total_cp_time/checkpoint_count);
    printf("FIrst CP time: %f\n",first_cp_time);
    printf("Total PULL time: %f\n",total_pull_time);
    printf("Total FLUSH time: %f\n",total_flush_time);
   // printf("Avg PULL time: %f\n",total_pull_time/checkpoint_count);
    printf("Total wu time: %f\n",total_wu);
    printf("Total FP_BP time: %f\n",total_fp_bp);
    
    printf("Total data_prep time: %f\n",total_data_prep_time);
    printf("Total process time: %f \n",what_time_is_it_now()- total_trainning_time_start);
    #ifdef PROFILE
    cudaProfilerStop();
    #endif
...
#endif
    char buff[256];
    sprintf(buff, "%s/%s_final.weights", "/lclhome/aghos008", base);
