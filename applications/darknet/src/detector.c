...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, float thresh, float iou_thresh, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    ...
    for (k = 0; k < ngpus; ++k) {
        ...
        if (weightfile) {
           ...
           // double restore_st = what_time_is_it_now();
            load_weights_libpm(&nets[k], cont,  CONTAINER_NUMBER);
           // double restore_et = what_time_is_it_now();
           // printf("libpm restore time: %f \n", restore_et-restore_st);
            return;
...
*/
    ...
    while (get_current_iteration(net) < net.max_batches) {
...
#endif
        total_trainning_time = total_trainning_time +  (what_time_is_it_now()-time);
     ...
     //   printf("\n %d: %f, %f avg loss, %f rate, %lf seconds, %d images, %f hours left\n", iteration, loss, avg_loss, get_current_rate(net), (what_time_is_it_now() - time), iteration*imgs, avg_time);
        ...
        //draw_train_loss(windows_name, img, img_size, avg_loss, max_img_loss, iteration, net.max_batches, mean_average_precision, draw_precision, "mAP%", avg_contrastive_acc / 100, dont_show, mjpeg_port, avg_time);
          ...
          if (iteration >= (iter_save_last + cp_freq)) {
            ...
            double cp_time = 0;
            #ifdef LIBPM
            int new_pages = save_weights_libpm(net, buff,is_initial_checkpoint,cont,CONTAINER_NUMBER, &pull_time, &cp_time);
            if(is_initial_checkpoint)
                first_cp_time  = cp_time;
            //printf("libpm checkpoint time: %f   new pages: %d \n", cp_time,new_pages);
            #else
            //sprintf(buff, "%s/%s_final.weights", backup_directory, base);
            sprintf(buff, "%s/%s_final.weights", "/mnt/nvme0n1/", base);
            //sprintf(buff, "%s/%s_final.weights", "/mnt/pmem/", base);            
            save_weights_2(net, buff,&pull_time, &cp_time);
           
            if(is_initial_checkpoint)
                first_cp_time  = cp_time;
           //printf("file checkpoint time: %f \n", cp_time);
            #endif
            if(!is_initial_checkpoint)
            {
    ...
    printf("Checkpoint count: %d\n",checkpoint_count);
        ...
        printf("Total trainning time: %f\n",total_trainning_time);
