...
struct page_allocator *page_allocator_init(unsigned int cid)
{
    ...
    pa->pa_handler = pa->pa_ops->init(cid);
...
void *page_allocator_get_contiguous_pages(unsigned int cid, int npages, size_t *laddr, int flags)
{
    STATS_INC_ALLOCPG();
    struct page_allocator *pa;
    pa = get_page_allocator(cid);
    allocated_page_count += npages;
    return pa->pa_ops->alloc_contiguous_pages(pa->pa_handler,  npages, laddr, flags);
}
...
void *page_allocator_mappage_with_prot(unsigned int cid, size_t laddr,int prot)
{
    struct page_allocator *pa;
    pa = get_page_allocator(cid);
    return pa->pa_ops->map_page_with_prot(pa->pa_handler, laddr,prot);
}

void *get_current_page_allocator(unsigned int cid)
{
    return get_page_allocator(cid);
}
...
void page_allocator_mprotect_generic(void *maddr, size_t size, int flags)
{
    ...
    int  res = mprotect(maddr, size, flags);
    if (res)
    {
        if(errno == EACCES)
            printf("Eaccess\n");
        ...
        handle_error("failed to memprotect page %p\n,maddr");
    }
