...
int slab_entry_init(unsigned int cid, struct slab_entry *se, int size)
{
    ...
    {
        se->se_data.current.maddr = page_allocator_get_contiguous_pages(cid, 
        ROUNDPG(size)/PAGE_SIZE , &se->se_data.current.laddr,PA_PROT_READ);
    }
...
static void do_fixptrs_with_pointer_pages(unsigned int cid)
{
    struct slab_dir *sd = get_container(cid)->current_slab.maddr;
    struct pointer_locations_page* plp =  sd->pointer_locations_page.maddr;
    int total_entries = sd->total_pointers_size;

    // for(int  i = 0; total_entries > (i* POINTER_ENTRIES_SIZE); i++)
    // {
    ...
    //     {
    //         void** ptr_loc = page_allocator_mappage_with_prot(cid,page->pointer_laddr[j],PROT_WRITE);
    //         void *ptr_val = *ptr_loc;
            
    //         if(page->pval_offset[j] != SLAB_PTR_OFFSET_NULL &&
    //         page->pval_slab_entry[j] != SLAB_PTR_SEID_NULL)
    //         {
    //             struct slab_entry *se_val;
    //             se_val = get_slab_entry_by_id(cid, page->pval_slab_entry[j]);
    //             if(!get_container(cid)->is_not_head && 0)
    //                  *ptr_loc = (void*) se_val->se_data.current.maddr + page->pval_offset[j] ;
    //             else
    //                  atomic_set(ptr_loc , (void*) se_val->se_data.current.maddr + page->pval_offset[j] );
    //         }
    //         else
    //             ptr_val = NULL;   
    //     }
    ...
    for(int  i = 0; total_entries > (i* POINTER_ENTRIES_SIZE); i++)
    {
        struct pointer_page* page = (struct pointer_page*) 
            plp->pointer_pages[i].maddr;
        // printf("POINTER_ENTRIES_SIZE:%d\n", POINTER_ENTRIES_SIZE);
        for(int j = 0; j < POINTER_ENTRIES_SIZE; j++)
        {   
            if (total_entries <= ((i* POINTER_ENTRIES_SIZE)+j)) 
                return;
            ...
            for (int k = 0; k < si->si_index; k++) {
                ...
                for (int l = 0; l < SLAB_BUCKET_ENTRIES; l++) {
                    ...
                    if (!SLAB_ENTRY_IS_INIT(se_loc))
                        continue;
                    //todo ashikee: Temporary fix. needs to be fixed.
                }
            }
        }
    }
...
void slab_fixptrs(unsigned int cid) { 
     ...
     do_fixptrs_with_pointer_pages(cid);
}

static void do_insert_pointer(unsigned int cid, void **ptr_loc)
{
    int is_found = htable_find(get_ht_pointerat(),ptr_loc);
    if(is_found == PERSISTED_POINTER)
    {
        
        struct slab_dir *sd = get_container(cid)->current_slab.maddr;
        struct slab_entry  *se_val = NULL;
        size_t ptr_laddr = ptoi(ptr_loc)- ptoi(get_container(cid));
        void *ptr_val = *ptr_loc;
        if(ptr_val != NULL)
        {
            assert(ptr_loc && "The location of the pointer must not be NULL");

            void *rounded_ptr_loc = itop(ROUND_DWNPG(ptoi(ptr_val)));
            ...
            if (se_val) {
                uint16_t pval_offset = (ptoi(ptr_val) - ROUND_DWNPG(ptoi(ptr_val)));
                insert_pointer_location(cid,ptr_laddr,se_val->se_id,pval_offset);
            }
           
        }
        else
        {
            insert_pointer_location(cid,ptoi(ptr_loc)- ptoi(get_container(cid)),SLAB_PTR_SEID_NULL,SLAB_PTR_OFFSET_NULL);


        }
         htable_insert_2(get_ht_pointerat(),ptr_loc,ptr_loc,2);
...
size_t slab_setroot(unsigned int cid, void *maddr)
{
    ...
    if (se) {
        ...
        //uint64_t offset = ROUND_DWNPG(ptoi(maddr)) - ptoi(maddr);
       uint64_t offset = ptoi(maddr) - ROUND_DWNPG(ptoi(maddr));

        PACK_CONT_ROOT(&new_cont_root, se->se_id, offset);
