...
void *nlm_init()
{
    ...
    nlm->fd = open(cont_file_name, O_CREAT | O_RDWR, 0777);
    if (nlm->fd == -1)
...
void *nlp_mapper_alloc_contiguous_pages(void *handler, size_t n, size_t *laddr, int flags)
{
...
struct page_allocator_ops *nonlinear_mapper_ops()
{
    static struct page_allocator_ops ops = {
        ...
        .alloc_contiguous_pages = nlp_mapper_alloc_contiguous_pages,
