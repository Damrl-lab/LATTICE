...
#define MAX_FREE_PAGES_SEGMENT    ((PAGE_SIZE -sizeof(uint64_t)) / \
...
{
    uint32_t page_number[MAX_FREE_FIXED_PAGES];
    uint64_t next_free_page;

};

struct free_pages_segment
{
    uint32_t pgno[MAX_FREE_PAGES_SEGMENT];
    uint32_t page_count[MAX_FREE_PAGES_SEGMENT];
    uint64_t next_free_pages_segment;
};
