...
#define SLAB_LARGER_THAN_4K_ALLOC 4096
 ...
 */
#define SLAB_BUCKET_BITMAP_SIZE 16
#define SLAB_ENTRIES_DATA_BITMAP_SIZE_IN_SLAB_BUCKET 4
#define SLAB_ENTRIES_POINTERS_BITMAP_SIZE_IN_SLAB_BUCKET 4
#define SLAB_BUCKET_ENTRIES ((PAGE_SIZE - sizeof(unsigned int)-sizeof(char)) / sizeof(struct slab_entry))


struct slab_bucket {
    //todo: ashikee get rid of this sb_has_snapshot
    char modified;
 ...
 */
#define SLAB_INNER_MEM     ( 2* (sizeof(char) + sizeof(size_t) + sizeof(void*)))
#define SLAB_INNER_ENTRIES  ((PAGE_SIZE - sizeof(int) -  sizeof(char)) / SLAB_INNER_MEM)
...
#define SLAB_OUTER_ENTRIES  ((PAGE_SIZE - sizeof(int) -  sizeof(char)) / SLAB_OUTER_MEM)
#define SLAB_OUTER_FULL(p)  ((p)->so_index >= SLAB_OUTER_ENTRIES)
#define SLAB_OUTER_BITMAP_SIZE 16
...
#define SLAB_DIR_MEMSIZE    (sizeof(int) +          /* sd_index */ \
                                1 * sizeof(char) + /* sd_modified */ \
                                3 * sizeof(void*) + /* sd_maddr_root */ \
                                3 * sizeof(void*) + /* sd_size_root */ \
                                2 * sizeof(void*) + /* sd_free_list */ \
                                1 * sizeof(void*) + /* container_begin_addrs */ \
                                1 * sizeof(void*) + /* pointers_size */ \
                                 2 * sizeof(void*) + /* pointer_locations_page */ \
                                  2 * sizeof(void*) + /* snapshot_pointer_locations_page */ \
                                sizeof(struct pptr_head) + /* sd_ptr_list_head */ \
                                sizeof(uint64_t))   /* sd_cont_root */
#define SLAB_DIR_ENTRIES    ((PAGE_SIZE - SLAB_DIR_MEMSIZE -(2 * SLAB_OUTER_BITMAP_SIZE) ) / \
...
struct slab_dir {
    ...
    bitstr_t so_bitmap[SLAB_OUTER_BITMAP_SIZE];
    struct {
        struct pointer_locations_page *maddr;
        size_t laddr;
    } pointer_locations_page;
    struct {
        struct pointer_locations_page *maddr;
        size_t laddr;
    } snapshot_pointer_locations_page;
    unsigned long total_pointers_size;
    VECTOR_DECL(se_vector, struct slab_entry*) sd_vector;
    void* container_begin_addr_in_last_commit;
};
