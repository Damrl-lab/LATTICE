...
void handle_memory_update(int sigid, siginfo_t *sig, void *unused)
{
    ...
    if(get_container(cid)->is_not_head || is_restoring())
    {
       
        page_allocator_mprotect(cid, pgaddr, PAGE_SIZE, PA_PROT_READ | PA_PROT_WRITE);
        return;
    ...
    if(!se)
    {
        struct slab_entry *se_pivot;
        RB_FOREACH(se_pivot, used_slab_entry_tree, &sd->sd_maddr_root) {
            if(se_pivot->se_size >= PAGE_SIZE)
            {
                if(pgaddr >= se_pivot->se_data.current.maddr && pgaddr <= 
                itop(ptoi(se_pivot->se_data.current.maddr) + se_pivot->se_size)) 
                {
                    se = se_pivot;
                    break;
                }
    ...
    if (se) {
        ...
        int npages = ROUNDPG(se->se_size)/PAGE_SIZE;

        page_allocator_mprotect(cid, se->se_data.current.maddr, PAGE_SIZE*npages, PA_PROT_READ | PA_PROT_WRITE);
        struct slab_bucket* sb = (struct slab_bucket*) ROUND_DWNPG(ptoi(se));
       ...
       //todo: ashikee shortcut now pages not found in se considered as metadata
        page_allocator_mprotect(cid, pgaddr, PAGE_SIZE, PA_PROT_READ | PA_PROT_WRITE);
...
void slab_entry_snapshot(unsigned int cid, struct slab_entry *se)
{
  ...
  /*      size_t data_laddr;
        void *data_maddr;
        if(se->se_size < PAGE_SIZE)
          data_maddr = page_allocator_getpage(cid, &data_laddr, PA_PROT_WRITE);
        else
        {
            data_maddr = page_allocator_get_contiguous_pages(cid, 
            ROUNDPG(se->se_size)/PAGE_SIZE,&data_laddr, PA_PROT_WRITE);
