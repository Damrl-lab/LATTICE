...
struct htable *ht_pointerat;
struct htable *ht_mallocat;
/*
 * 1. init subsystem
...
#endif
    int is_found = htable_find(ht_pointerat,ptr_loc);
    if(!is_found)
        htable_insert_2(ht_pointerat, ptr_loc, ptr_loc,NEWLY_ALLOCATED_POINTER);//new pointer location
    else if(is_found == 2)
      htable_insert_2(ht_pointerat, ptr_loc, ptr_loc,MODIFIED_EXISTING_POINTER);//modified existing
...
void print_htables()
{
    printf("printing ht_mallocat\n");
    htable_print(ht_mallocat);
    printf("\n\n");
    printf("printing ht_pointerat\n\n");
 ...
 */
static int classify_pointers_callback(void *key, void *val,int val2, void *param)
{
    ...
    if ((se = slab_find(cid, ptr_loc))) {
         ...
         * allocations
         * returns 1, so remove from ht
         */
       int is_found = htable_find(ht_pointerat,ptr_loc);
       if(is_found == NEWLY_ALLOCATED_POINTER || is_found == MODIFIED_EXISTING_POINTER)
            VECTOR_APPEND(&pptr_vec, ptr_loc);
        if(is_found != MODIFIED_EXISTING_POINTER)
        return 1;
        else return 0;
    }
    ...
    if (e) {
        if (!e->paddr) {
            ...
            VECTOR_FOREACH(&e->ptrs, ptr, i) {
                ...
                htable_insert_2(ht_pointerat,
                PTR_REBASE(e->addr, ptr, e->paddr),
                PTR_REBASE(e->addr, ptr, e->paddr),3);
            }
        }
...
void move_volatile_allocations(unsigned int cid)
{
    ...
    for (int i = 0; i < VECTOR_SIZE(&pptr_vec); i++) {
        ...
        int is_found = htable_find(ht_pointerat,ptr_loc);
        if(is_found == NEWLY_ALLOCATED_POINTER || is_found == MODIFIED_EXISTING_POINTER)
            htable_insert_2(ht_pointerat,ptr_loc,ptr_loc,PERSISTED_POINTER);
        slab_insert_pointer(cid, ptr_loc); // // do_insert_pointer
    }
...
static void fix_back_references_callback(void *key, void *val, void *_not_use_)
{
    ...
    if (e) {
        *ptr_loc = PTR_REBASE(e->addr, *ptr_loc, e->paddr);
        ...
        //todo: Ashikee handle this issue
...
void insert_existing_pointer_ats(int cid)
{
