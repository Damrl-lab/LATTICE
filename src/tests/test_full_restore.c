...
#define PACK_EIGHT_NODES_PER_SLABENTRY    ((PAGE_SIZE / 8) - (sizeof(int) + sizeof(void*)) - 10)
#define PACK_SIXTEEN_NODES_PER_SLABENTRY    ((PAGE_SIZE / 16) - (sizeof(int) + sizeof(void*)) - 10)
...
struct input_options {
    ...
    .num_nodes = 40,
    .node_size = PACK_SIXTEEN_NODES_PER_SLABENTRY,
...
struct node {
    int data;
    struct node *next;
    char _c[PACK_FOUR_NODES_PER_SLABENTRY];
};
...
void create_list(int nodes_count)
{
    ...
    if(res == 0)
        printf("previous containter deleted\n");
  ...
  //  container_test_pointer_insert(cont->id, cont_root_addr); 
  //  container_test_pointer_delete(cont->id, cont_root_addr); 
    container_cpoint(cont->id);
...
void restore_list()
{
    ...
    printf("starting restore op:\n");
    cont = container_restore(cid);
    printf("restore finished\n");

    //todo ashikee: temp disabled. Needs to be fixed.
    n = container_getroot(cid);
    printf("container root set\n");
...
//    container_cpoint(cont->id);
//    printf("Data checkpointed again\n");
    printf("\n\n");
}
void add_after_restore()
{
    int cid = 0;
    struct container *cont;
    struct node *n;
    ...
    do {
        ...
        switch(next_option) {
            ...
            case '?': /* user provided invalid option */
                //todo ashikee set data for add after restore
                 add_after_restore();
