...
struct htable_entry *htable_entry_alloc_2(void *key, void *val,int v2, uint32_t hash)
{
    struct htable_entry *e = malloc(sizeof(*e));
    assert(e && "failed to allocate memory for htable_entry");
    e->key = key;
...
int htable_insert_2(struct htable *ht, void *key, void *val, int v2)
{
    ...
    if (*ptr == NULL) {
        struct htable_entry *new_entry = htable_entry_alloc_2(key, val,v2, hash);
        *ptr = new_entry;
        ret = 1;
...
void htable_print(struct htable *ht)
{
    ...
    for (int i = 0; i < ht->length; i++) {
        struct htable_entry *e = ht->list[i];
        while (e != NULL) {
            printf("htable_entry { hash: %x, key: %p, val: %p, val2: %d }\n",
                    e->hash, e->key, e->val, e->val2);
            e = e->next_hash;
            count++;
        }
    }
...
void htable_filter(struct htable *ht,
                    int (*fun)(void *key, void *val, int val2, void *param),
                    void *param)
{
    for (int i = 0; i < ht->length; i++) {
        struct htable_entry *e = ht->list[i];
        ...
        while (e != NULL) {
            if (fun(e->key, e->val,e->val2, param)) {
                ht->elems--;
                *ptr = e->next_hash;
                free(e);
                e = *ptr;
